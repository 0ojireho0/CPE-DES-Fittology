...
class poseDetectorHighKneeTap():
    ...
    def get_pose_orientation(self, landmarks):
        ...
        if shoulder_x[0] <= -0.10 and shoulder_x[1] >= 0.05 and hip_x[0] <= -0.05 and hip_x[1] >= 0.05:
            return 'right'
        elif shoulder_x[0] >= 0.03 and shoulder_x[1] >= -0.25 and hip_x[0] >= 0.05 and hip_x[1] <= -0.05:
            return 'left'
        elif -0.20 <= shoulder_x[0] <= 0.15 and -0.15 <= shoulder_x[1] <= 0.10 and -0.03 <= hip_x[0] <= 0.03 and -0.03 <= hip_x[1] <= 0.03 :
            return 'front'
        else:
            return 'none'
