...
class poseDetectorBodyHipHinge():
    ...
    def get_pose_orientation(self, landmarks):
        ...
        if shoulder_x[0] <= -0.10 and shoulder_x[1] >= 0.03 and hip_x[0] <= -0.05 and hip_x[1] >= 0.05:
            return 'right'
        elif shoulder_x[0] >= 0.03 and shoulder_x[1] <= -0.10 and hip_x[0] >= 0.05 and hip_x[1] <= -0.05:
            return 'left'
        elif shoulder_x[0] <= -0.01 and shoulder_x[1] <= -0.01 and -0.01 <= hip_x[0] <= 0.01 and -0.01 <= hip_x[1] <= 0.01 :
            return 'front'
        else:
            return 'none'
        ...
        if len(self.lmList) != 0:
            ...
            if orientation == 'right':
                ...
                if drawpoints:
                    ...
                    cv2.line(img, (x1, y1), (x3, y3), (0, 0, 255), 6)
...
def main():
    ...
    pTime = 0
    detector = poseDetectorBodyHipHinge()
    while True:
        success, img = cap.read()
        img = detector.findPose(img)
        lmList = detector.findPosition(img, draw=False)

        # Analyze pose using HipHinge method
        if lmList:
            measurement, orientation = detector.HipHinge(img, 11, 23, 25, drawpoints=True)
            measurement, orientation = detector.HipHinge(img, 12, 24, 26, drawpoints=True)
