...
class poseDetectorGobletSquat():
    ...
    def get_pose_orientation(self, landmarks):
        ...
        if shoulder_x[0] <= -0.10 and shoulder_x[1] >= 0.05 and hip_x[0] <= -0.05 and hip_x[1] >= 0.05:
            return 'right'
        elif shoulder_x[0] >= 0.03 and shoulder_x[1] >= -0.21 and hip_x[0] >= 0.05 and hip_x[1] <= -0.05:
            return 'left'
        elif -0.15 <= shoulder_x[0] <= 0.05 and -0.18 <= shoulder_x[1] <= 0.07 and -0.01 <= hip_x[0] <= 0.01 and -0.01 <= hip_x[1] <= 0.01 :
            return 'front'
        else:
            return 'none'
